...
import { CONVERSION_STAMP, USD_TO_INR } from './currency';
import type { Currency } from './currency';

const PCT_FMT = '0.0%';

/** Excel number format per currency ($ US grouping vs ₹ Indian lakh-crore). */
function moneyFmt(currency: Currency): string {
...
export function buildProposalWorkbook(model: ProposalModel): XLSX.WorkBook {
  ...
  // All money is authored in USD; convert to the selected display currency so
  // the spreadsheet's numbers (and SUM formulas) match the on-screen figures.
  const cur = meta.displayCurrency;
  const rate = cur === 'INR' ? USD_TO_INR : 1;
  const MONEY_FMT = moneyFmt(cur);
  const m = (n: number) => n * rate;
  ...
  const summaryAoa: (string | number)[][] = [
    ...
    ['Currency', cur],
    ['Conversion', cur === 'INR' ? CONVERSION_STAMP : '—'],
    [],
    ['Monthly total', m(costs.total)],
    ['Annual (×12)', m(costs.annual)],
    ['One-time setup', m(costs.setup)],
    [],
  ...
  // Monthly / Annual / One-time now sit one row lower (Conversion row added).
  setFmt(wsSummary, 'B9', MONEY_FMT);
  setFmt(wsSummary, 'B10', MONEY_FMT);
  setFmt(wsSummary, 'B11', MONEY_FMT);
  XLSX.utils.book_append_sheet(wb, wsSummary, 'Summary');

  // ---- (b) Cost Breakdown ---------------------------------------------
  const header = ['Cost bucket', `${cur}/mo`, `${cur}/yr`, '% of total', 'Custom'];
  ...
  const monthlyMisc = model.misc.filter((mi) => mi.monthly > 0);
  const oneTimeMisc = model.misc.filter((mi) => mi.oneTime > 0);
  monthlyMisc.forEach((mi) => {
    rows.push([
      `Misc: ${mi.label}`,
      m(mi.monthly),
      m(mi.monthly * 12),
      mi.monthly / (costs.total || 1), // fraction for 0.0% format
      '',
    ]);
  });
  ...
  wsBreak[`B${setupRow}`] = { t: 'n', v: m(costs.setup), z: MONEY_FMT };
  lastRow = setupRow;
  oneTimeMisc.forEach((mi, i) => {
    const rr = setupRow + 1 + i;
    XLSX.utils.sheet_add_aoa(wsBreak, [[`One-time: ${mi.label}`]], { origin: `A${rr}` });
    wsBreak[`B${rr}`] = { t: 'n', v: m(mi.oneTime), z: MONEY_FMT };
    lastRow = rr;
  });
  ...
  for (let r = firstDataRow; r <= lastDataRow; r++) {
    setFmt(wsBreak, `B${r}`, MONEY_FMT);
    setFmt(wsBreak, `C${r}`, MONEY_FMT);
  ...
  if (model.misc.length === 0) {
    ...
    model.misc.forEach((mi) =>
      miscAoa.push([mi.label, m(mi.oneTime || mi.monthly), mi.oneTime ? 'One-time' : 'Monthly']),
    );
  }
  const wsMisc = XLSX.utils.aoa_to_sheet(miscAoa);
  for (let r = 2; r <= miscAoa.length; r++) setFmt(wsMisc, `B${r}`, MONEY_FMT);
